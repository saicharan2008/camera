...
function setup() {
  ...
  trex = createSprite(10,height-20,20,50);
  ...
  gameover = createSprite(displayWidth-300,displayHeight-140,10,10);
  gameover.addImage(gameovering)
  gameover.scale=3.5;
  gamerestart = createSprite(displayWidth-300,displayHeight-100,10,10);
  gamerestart.addImage(gamerestarting)
  gamerestart.scale=0.5;
  invisibleGround = createSprite(100,displayHeight-10,4000,10);
...
camera.position.y=displayHeight/2
  ...
  if(gameState === PLAY){
    ...
    if(score%100===0 && score>0){
     ...
     spawnObstacles();
     if((touches.length>0||keyDown("space"))&& trex.y >= displayHeight-100) {
    trex.velocityY = -13;
    jump.play();
      touches=[] 
  }
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(displayWidth-200,displayHeight-35,10,40);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(displayHeight-190,displayHeight-140));
